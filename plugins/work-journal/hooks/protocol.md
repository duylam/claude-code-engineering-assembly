...
Two append-only logs, driven **only** through the `work-journal` skills (never touch the logs by
...
1. `work-journal:diary-reader --session-brief` — open items grouped by work item, plus the recent
   entry headers. This is where the last session left off.
2. `work-journal:decision-reader --last 10` — the recent reasoning.
...
1. `work-journal:diary-writer` with `Did / Why / Next`, always passing:
...
2. If the work embodied a decision (tech choice, changed scope, reversed plan, non-obvious
   trade-off), `work-journal:decision-writer` with `Consideration / Decision / Rationale`
